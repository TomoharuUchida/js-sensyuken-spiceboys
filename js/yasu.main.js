...
$('#shopping_add_y').on('click', function () { //shopping_addボタンが押されたら  
  $('#shopping_output_y').empty(); //再表示のために一回消す
  const text = $('#shopping_input_y').val(); //入力されたものを変数に入れる

  $("#shopping_input_y").val("");//入力欄を空欄にする
  $("#shopping_input_y").focus();//入力欄にフォーカスを当てる

  //変数textを配列に入れる
  shoppingArray.push(`<div class="shopping_txt_y"><label><input type="checkbox" class="check_y">${text}</label><button class= "delete_btn_y fas fa-trash-alt"></button></div>`);
  ...
  //ページに表示
  $('#shopping_output_y').html(shoppingArray);//htmlに変換
});


//ゴミ箱ボタンでtodoの項目を消す
$(document).on('click', '.delete_btn_y', function () { //deleteボタンが押されたら
  ...
  $('#shopping_output_y').html(shoppingArray);//配列を再表示
  //↑htmlを置き換える
...
// shoppinglistデータの削除
$("#clear_btn_y").on("click", function () {
  localStorage.removeItem("memo_todo");//キーmemo_todoをローカルストレージから削除
  $("#shopping_input_y").val(""); //入力欄を空に
  $('#shopping_output_y').empty(); //画面に表示されているものをを全部消す
